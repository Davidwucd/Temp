...
def text8_corpus(path):
    # A generator that returns lists of tokens representing individual words in the comment
    return (tokenize_comment(comment) for comment in extract_tex8(path))
...
def generate_output(vocab_list, vocab_emb, output_file):
    # Generate the word-embedding output with given format
    with open(self._save_path+"/vec_glove.txt", "w") as f:
    for i in range(vocab_list.shape[0]):
		f.write("%s "%(vocab_list[i]))
		for j in range(vocab_emb.shape[1]):
			f.write("%f "%(vocab_emb[i,j]))
			f.write("\n")
...
if __name__ == "__main__":
	...
	corpus = text8_corpus(text_path)
	model = GloVeModel(embedding_size=200, context_size=10, min_occurrences=25,
						learning_rate=0.05, batch_size=512)
	model.fit_to_corpus(corpus)
	model.train(num_epochs=50, log_dir="log/example", summary_batch_interval=1000)
	
	vocab_list = np.genfromtxt(fname=vocab_path, dtype='str')
	vocab_emb = []
	for vocab in vocab_list:
		word_emb = [model.embeddings[model.id_for_word(vocab)]]
		vocab_emb = np.append(vocab_emb, word_emb)
	generate_output(vocab_list, vocab_emb, output_path)
